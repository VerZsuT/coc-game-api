...
function getPlayersAPI(token: string): PlayersAPI {
  return {
    raw: {
      ...
      verifyToken: (tag, playerAPIToken) => verifyToken(token, tag, playerAPIToken),
    },
    async getByTag(tag) {
      const data = await getPlayerInfo(token, tag)
      return {
        ...data,
        clan: data.clan ? clanHelpers.createClanExtObject(token, data.clan.tag, data.clan) : undefined,
      }
    },
